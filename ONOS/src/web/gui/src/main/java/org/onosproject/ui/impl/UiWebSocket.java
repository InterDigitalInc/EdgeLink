...
public class UiWebSocket
            ...
            if (factory != null) {
                factory.newHandlers().forEach(handler -> {
                    try {
                        ...
                        handler.messageTypes().forEach(type -> {
                            if(type.equalsIgnoreCase("nbiNotification")) {
                                log.info("contains");
                                NbiNotificationHandler.getInstance().addUiMessageHandler(handler);
                            }
    ...
    private synchronized void destroyHandlersAndOverlays() {
        ...
        handlers.forEach((type, handler) -> {
            if(type.equalsIgnoreCase("nbiNotification")) {
                log.info("contains");
                NbiNotificationHandler.getInstance().removeUiMessageHandler(handler);
            }
            handler.destroy();
        });
        handlers.clear();
