                ...
                try {
                    ...
                    if (storedRule != null) {
                        if (storedRule.exactMatch(rule)) {
                            ...
                            //exception: if switches are allowed control on adding rules, they are also
                            //permitted to modify an action and keep it as is without intervention from the flow manager
                            if(!allowExtraneousRules) {
                                extraneousFlow(rule);
                                flowMissing(storedRule);
                            } else {
                                log.debug("flow rules are not an exact match, but we keep the difference as is, we do not intervene, the switch knows better!");
                            }
                        }
                    } else {
